...
#  Copyright 2022 Chriss IT. Leong
#
...
from ccg2lambda_tools_wsc import assign_semantics_to_ccg
from semantic_index_wsc import SemanticIndex
...
def serialize_tree(tree):
    semnodes = tree.findall('.//semantics[@status="success"]')
    tree_str = []
    if semnodes:
        for semnode in semnodes:
            root = semnode.get('root')
            # semnode.find('.//span[@id="s0_sp0"]').get('sem')
            tree_str.append(
                semnode.find(('.//span[@id="%s"]' % root)).get('sem')
            )
    else:
        tree_str.append('Failed')
