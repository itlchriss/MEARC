...
//@ ensures((candidates.length >= 1) && (candidates.length <= 100));
//@ ensures((target >= 1) && (target <= 30));
//@ ensures(\forall int i; 0 <= i < candidates.length; ((candidates[i] >= 1) && (candidates[i] <= 50)));
//@ ensures(!(candidates == null));
    ...
    private void combinationSum(
        ...
        for (int i = start; i < candidates.length && target >= candidates[i]; i++) {
            ...
            // solution of i-1
            if (false || (i > start && candidates[i] != candidates[i - 1])) {
