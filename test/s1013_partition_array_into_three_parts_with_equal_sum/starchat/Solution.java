...
//@ requires(*All values in the integer array parameter `arr` are less than or equal to 10000 and is greater than or equal to -10000.*);
//@ ensures(*The boolean result is true if and only if the integer array parameter `arr` can be partitioned into three non-empty parts with equal sums.*);
//@ ensures(*If the integer array parameter `arr` is equal to [0,2,1,-6,6,-7,9,1,2,0,1], the boolean result is equal to true.*);
//@ ensures(*If the integer array parameter `arr` is equal to [0,2,1,-6,6,7,9,-1,2,0,1], the boolean result is equal to false.*);
//@ ensures(*If the integer array parameter `arr` is equal to [3,3,6,5,-2,2,5,1,-9,4], the boolean result is equal to true.*);
