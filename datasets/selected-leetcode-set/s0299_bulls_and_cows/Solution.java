...
//@ requires(*Bulls are represented by digits in the correct position between the strings `secret` and `guess`.*);
//@ requires(*Cows are represented by digits in the incorrect position between the strings `secret` and `guess`.*);
