...
//@ ensures(*The integer array parameter `words` must not be null.*);
//@ ensures(*The integer parameter `maxWidth` is greater than or equal to 1 and is less than or equal to 100.*);
//@ ensures(*The length of the string list result is greater than or equal to 1 and is less than or equal to 300.*);
//@ ensures(*Each string in the string list result has a length greater than 0 and does not exceed `maxWidth`.*);
//@ ensures(*Extra spaces between words in each line are distributed as evenly as possible.*);
//@ ensures(*If the number of spaces on a line does not divide evenly between words, the empty slots on the left will be assigned more spaces than the slots on the right.*);
//@ ensures(*The last line of text is left-justified and no extra space is inserted between words.*);
    public List<String> fullJustify(String[] words, int maxWidth) {
        ...
        // overflows
        //@ loop_invariant 0 <= i < words.length;
