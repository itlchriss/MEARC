...
//@ ensures(*The integer result is the area of the largest rectangle containing only '1's in the binary matrix.*);
//@ ensures(*The binary matrix parameter `matrix` must not be null.*);
//@ ensures(*The binary matrix parameter `matrix` must be a `rows x cols` matrix.*);
//@ ensures(*The binary matrix parameter `matrix` must only contain '0's and '1's.*);
//@ ensures(*The integer result is greater than or equal to 0.*);
//@ ensures(*The integer result is less than or equal to 200.*);
