...
//@ ensures(*Half of the integers in `nums` are even.*);
//@ ensures(*The integers in `nums` are between 0 and 1000 (inclusive).*);
//@ ensures(**);
...
//@ ensures(*The integers in the output array are sorted such that whenever `nums[i]` is odd, `i` is odd, and whenever `nums[i]` is even, `i` is even.*);
    public int[] sortArrayByParityII(int[] nums) {
        ...
        int j = 1;
        //@ havoc i, j;
        //@ loop_writes i, j, nums;
        //@ maintaining 0 <= i <= nums.length;
        //@ maintaining 1 <= j <= nums.length;
