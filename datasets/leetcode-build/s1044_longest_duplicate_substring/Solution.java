...
public class Solution {
	...
	//@ requires(*The length of `s` is at least 2 and at most 3 * 10^4.*);
	//@ requires(*`s` consists only of lowercase English letters.*);
	//@ ensures(*The method returns a string that represents a duplicated substring of `s`.*);
	//@ ensures(*The returned string has the longest possible length among all duplicated substrings of `s`.*);
	//@ ensures(*If `s` does not have a duplicated substring, the method returns an empty string "".*);
