...
public class Solution {
	...
	//@ requires(*The input matrix `mat` is a 2D array of integers.*);
	//@ requires(*The input matrix `mat` has at least one row and one column.*);
	//@ requires(*The input matrix `mat` has dimensions `m x n`, where `m` is the number of rows and `n` is the number of columns.*);
	//@ requires(*The values in the input matrix `mat` are positive integers.*);
