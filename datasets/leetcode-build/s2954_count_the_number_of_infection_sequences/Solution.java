...
public class Solution {
	...
	//@ requires(*The input integer `n` must be greater than or equal to 2.*);
	//@ requires(*The length of the input array `sick` must be greater than or equal to 1.*);
	//@ requires(*The elements in the input array `sick` must be in the range from 0 to `n - 1`.*);
	//@ requires(*The input array `sick` must be sorted in increasing order.*);
	//@ ensures(*The output integer must be the total number of possible infection sequences.*);
	//@ ensures(*The output integer must be returned modulo 10^9 + 7.*);
