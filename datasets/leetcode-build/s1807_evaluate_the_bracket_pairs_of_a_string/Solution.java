...
public class Solution {
	...
	//@ requires(*The input list `knowledge` is not null.*);
	//@ requires(*Each element in `knowledge` is a list of size 2.*);
	//@ requires(*The length of each key and value in `knowledge` is between 1 and 10.*);
	//@ requires(*The input string `s` consists of lowercase English letters and round brackets.*);
	//@ requires(*Every open bracket in `s` has a corresponding close bracket.*);
	//@ requires(*The key in each bracket pair of `s` is non-empty.*);
	//@ requires(*Each key in `knowledge` is unique.*);
