...
public class Solution {
	//@ requires(*The input strings `s` and `p` are not null.*);
	//@ requires(*The length of `s` is greater than or equal to the length of `p`.*);
	//@ ensures(*The method returns a list of integers representing the start indices of `p`'s anagrams in `s`.*);
	//@ ensures(*The order of the indices in the returned list can be in any order.*);
