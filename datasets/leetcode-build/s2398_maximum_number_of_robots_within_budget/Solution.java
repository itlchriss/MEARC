...
public class Solution {
	...
	//@ requires(*The length of `chargeTimes` and `runningCosts` arrays must be equal to `n`.*);
	//@ requires(*The length of `chargeTimes` and `runningCosts` arrays must be greater than 0.*);
	//@ requires(*The elements in `chargeTimes` and `runningCosts` arrays must be positive integers.*);
	//@ requires(*The value of `budget` must be a positive integer.*);
	//@ ensures(*The returned value must be an integer.*);
	//@ ensures(*The returned value must be the maximum number of consecutive robots that can be run without exceeding the budget.*);
