...
public class Solution {
	//@ requires(*The array `digits` is not null.*);
	//@ requires(*The array `digits` is sorted in non-decreasing order.*);
	//@ requires(*The length of `digits` is between 1 and 9 (inclusive).*);
	//@ requires(*Each element in `digits` is a single digit from '1' to '9'.*);
	//@ requires(*The integer `n` is between 1 and 10^9 (inclusive).*);
