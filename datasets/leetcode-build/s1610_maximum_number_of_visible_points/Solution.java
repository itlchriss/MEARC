...
public class Solution {
	//@ requires(*The input list `points` is not null.*);
	//@ requires(*The input list `points` contains at least one element.*);
	//@ requires(*The input integer `angle` is between 0 and 359 (inclusive).*);
	//@ requires(*The input list `location` is not null.*);
	//@ requires(*The input list `location` contains exactly two elements.*);
	//@ requires(*The input lists `points` and `location` contain only integers.*);
	//@ requires(*The coordinates in the input lists `points` and `location` are between 0 and 100 (inclusive).*);
	//@ ensures(*The output is an integer representing the maximum number of points that can be seen.*);
