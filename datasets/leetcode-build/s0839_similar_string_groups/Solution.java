...
public class Solution {
	//@ requires(*The input `strs` is not null.*);
	//@ requires(*The length of `strs` is greater than or equal to 1.*);
	//@ requires(*The length of each string in `strs` is greater than or equal to 1.*);
	//@ requires(*Each string in `strs` consists of lowercase letters only.*);
	//@ requires(*All words in `strs` have the same length and are anagrams of each other.*);
