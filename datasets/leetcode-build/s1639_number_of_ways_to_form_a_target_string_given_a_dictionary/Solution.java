...
public class Solution {
	...
	//@ requires(*The input `words` is a non-empty array of strings.*);
	//@ requires(*The input `target` is a non-empty string.*);
	//@ requires(*The length of each string in `words` is the same as the length of `target`.*);
	//@ requires(*All strings in `words` and `target` contain only lowercase English letters.*);
