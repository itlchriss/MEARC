...
public class Solution {
	...
	//@ requires(*The length of the input array `words` is greater than 0.*);
	//@ requires(*Each word in the input array `words` is not null.*);
	//@ requires(*Each word in the input array `words` is a character sequence consisting of non-space characters only.*);
	//@ requires(*Each word in the input array `words` has a length greater than 0 and not exceeding `maxWidth`.*);
	//@ requires(*The input `maxWidth` is greater than 0 and not exceeding 100.*);
	//@ requires(*The length of each word in the input array `words` is not exceeding `maxWidth`.*);
	//@ ensures(*The output is a list of strings.*);
	//@ ensures(*Each string in the output list has a length of exactly `maxWidth`.*);
	//@ ensures(*The words in each string in the output list are packed in a greedy approach, with as many words as possible in each line.*);
	//@ ensures(*Extra spaces between words in each string in the output list are distributed as evenly as possible.*);
	//@ ensures(*If the number of spaces on a line does not divide evenly between words, the empty slots on the left are assigned more spaces than the slots on the right.*);
	//@ ensures(*The last string in the output list is left-justified and does not have any extra space inserted between words.*);
