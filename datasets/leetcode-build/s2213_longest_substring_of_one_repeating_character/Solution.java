...
public class Solution {
	...
	//@ requires(*The input string `queryCharacters` is not null.*);
	//@ requires(*The input array `queryIndices` is not null.*);
	//@ requires(*The length of `s` is greater than or equal to 1 and less than or equal to 10^5.*);
	//@ requires(*The length of `queryCharacters` is equal to the length of `queryIndices` and is greater than or equal to 1 and less than or equal to 10^5.*);
	//@ requires(*The characters in `s` are all lowercase English letters.*);
	//@ requires(*The characters in `queryCharacters` are all lowercase English letters.*);
	//@ requires(*The values in `queryIndices` are valid indices within the range of `s`.*);
	//@ ensures(*The output array `lengths` is not null.*);
	//@ ensures(*The length of `lengths` is equal to the length of `queryCharacters`.*);
	//@ ensures(*The values in `lengths` represent the lengths of the longest substrings of `s` consisting of only one repeating character after each query is performed.*);
