...
public class Solution {
	...
	//@ requires(*The length of the input array `points` is greater than or equal to `k`.*);
	//@ requires(*Each element in the input array `points` is a valid point represented by an array of length 2.*);
	//@ requires(*The value of `k` is greater than or equal to 1.*);
	//@ ensures(*The output is a 2D array of size `k` x 2.*);
	//@ ensures(*Each element in the output array is a valid point represented by an array of length 2.*);
	//@ ensures(*The points in the output array are the `k` closest points to the origin.*);
	//@ ensures(*The order of the points in the output array is not specified.*);
