...
public class Solution {
	//@ requires(*The input arrays `aliceSizes` and `bobSizes` are not null.*);
	//@ requires(*The lengths of `aliceSizes` and `bobSizes` are greater than or equal to 1.*);
	//@ requires(*The elements of `aliceSizes` and `bobSizes` are positive integers.*);
	//@ requires(*The total number of candies in `aliceSizes` is not equal to the total number of candies in `bobSizes`.*);
