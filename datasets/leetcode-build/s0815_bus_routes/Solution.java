...
public class Solution {
	//@ requires(*The input array `routes` is not null.*);
	//@ requires(*The input array `routes` has at least one element.*);
	//@ requires(*The input array `routes` contains only unique values.*);
	//@ requires(*The input array `routes` has a length less than or equal to 500.*);
	//@ requires(*Each element in the input array `routes` is an array of integers.*);
	//@ requires(*Each element in the input array `routes` has a length less than or equal to 10^5.*);
	//@ requires(*The sum of the lengths of all elements in the input array `routes` is less than or equal to 10^5.*);
	//@ requires(*The input integers in the array `routes` are between 0 and 10^6.*);
	//@ requires(*The input integers `source` and `target` are between 0 and 10^6.*);
