...
public class Solution {
	...
	//@ requires(*The length of each element in `words` is exactly - Each element in `words` consists of lowercase English letters.*);
	//@ ensures(*The method returns an integer representing the length of the longest palindrome that can be created.*);
	//@ ensures(*If it is impossible to create any palindrome, the method returns 0.*);
