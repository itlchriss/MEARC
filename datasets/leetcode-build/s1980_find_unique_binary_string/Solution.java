...
public class Solution {
	...
	//@ requires(*Each string in `nums` is not null.*);
	//@ requires(*Each string in `nums` has a length of `n`.*);
	//@ requires(*Each string in `nums` consists of only `'0'` and `'1'`.*);
	//@ requires(*All strings in `nums` are unique.*);
