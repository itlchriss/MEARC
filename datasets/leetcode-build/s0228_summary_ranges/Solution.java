...
public class Solution {
	...
	//@ ensures(*Each element in the output list covers exactly one number from the input array.*);
	//@ ensures(*The output list is sorted in ascending order.*);
	//@ ensures(*Each range in the output list is represented as a string.*);
	//@ ensures(*If a range `[a,b]` has `a != b`, it is represented as `"a->b"`.*);
	//@ ensures(*If a range `[a,b]` has `a == b`, it is represented as `"a"`.*);
	//@ ensures(*There is no integer `x` such that `x` is in one of the ranges but not in the input array.*);
