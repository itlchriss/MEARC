...
public class Solution {
	//@ requires(*1. The input `cost` is a non-empty list of lists.*);
	//@ requires(*2. The length of `cost` is equal to `size1`.*);
	//@ requires(*3. Each sublist in `cost` has a length equal to `size2`.*);
	//@ requires(*4. `size1` is greater than or equal to `size2`.*);
	//@ requires(*5. The values in `cost` are non-negative integers.*);
	//@ requires(*6. The values in `cost` are less than or equal to 100.*);
	//@ ensures(*1. The method returns an integer representing the minimum cost to connect the two groups.*);
	//@ ensures(*2. The minimum cost is non-negative.*);
	//@ ensures(*3. The minimum cost is less than or equal to 100.*);
