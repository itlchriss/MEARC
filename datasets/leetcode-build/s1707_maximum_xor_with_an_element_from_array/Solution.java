...
public class Solution {
	...
	//@ requires(*The input arrays `nums` and `queries` are not null.*);
	//@ requires(*The length of `nums` and `queries` is greater than or equal to 1.*);
	//@ requires(*Each element in `nums` is a non-negative integer.*);
	//@ requires(*Each element in `queries` is an array of length 2.*);
	//@ requires(*The first element of each array in `queries` is a non-negative integer.*);
	//@ requires(*The second element of each array in `queries` is a non-negative integer.*);
