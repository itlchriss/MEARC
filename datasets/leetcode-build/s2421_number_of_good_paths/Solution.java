...
public class Solution {
	//@ requires(*The input array `vals` is not null.*);
	//@ requires(*The input array `edges` is not null.*);
	//@ requires(*The length of `vals` is equal to `n`.*);
	//@ requires(*The length of `edges` is equal to `n - 1`.*);
	//@ requires(*The values in `vals` are integers.*);
	//@ requires(*The values in `edges` are pairs of integers.*);
	//@ requires(*The values in `edges` are valid indices of nodes in `vals`.*);
	//@ requires(*The values in `edges` are distinct and do not contain duplicates.*);
	//@ requires(*The values in `edges` form a valid tree.*);
