...
public class Solution {
	//@ requires(*The `trips` array is not null.*);
	//@ requires(*The `capacity` is a positive integer.*);
	//@ requires(*The length of the `trips` array is between 1 and 1000.*);
	//@ requires(*Each trip in the `trips` array has a length of 3.*);
	//@ requires(*The number of passengers for each trip is between 1 and 100.*);
	//@ requires(*The pickup location for each trip is between 0 and 1000.*);
	//@ requires(*The drop-off location for each trip is between the pickup location and 1000.*);
