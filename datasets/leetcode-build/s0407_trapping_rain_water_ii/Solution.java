...
public class Solution {
    private static class Cell implements Comparable<Cell> {
        ...
        private int col;
        private int value;
	//@ requires(*The input matrix `heightMap` is not null.*);
	//@ requires(*The input matrix `heightMap` has at least one row and one column.*);
	//@ requires(*The input matrix `heightMap` has a valid height for each unit cell.*);
	//@ ensures(*The method returns an integer representing the volume of water trapped after raining.*);
