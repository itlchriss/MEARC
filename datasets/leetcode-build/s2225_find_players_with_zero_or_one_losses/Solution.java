...
public class Solution {
	//@ requires(*The input `matches` is a non-empty array.*);
	//@ requires(*Each element `matches[i]` is an array of length 2.*);
	//@ requires(*The values in `matches[i]` are integers.*);
	//@ requires(*The values in `matches[i]` are positive and less than or equal to 10^5.*);
	//@ requires(*The values in `matches[i]` are unique.*);
