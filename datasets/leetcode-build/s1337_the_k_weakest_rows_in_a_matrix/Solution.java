...
public class Solution {
	...
	//@ requires(*The input matrix `mat` has at least one row.*);
	//@ requires(*The input matrix `mat` has at least one column.*);
	//@ requires(*The input matrix `mat` contains only 0s and 1s.*);
	//@ requires(*The input integer `k` is greater than - The input integer `k` is less than or equal to the number of rows in `mat`.*);
