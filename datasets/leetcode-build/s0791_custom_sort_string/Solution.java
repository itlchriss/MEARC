...
public class Solution {
	//@ requires(*The input strings `order` and `s` are not null.*);
	//@ requires(*The length of `order` is between 1 and 26 (inclusive).*);
	//@ requires(*The length of `s` is between 1 and 200 (inclusive).*);
	//@ requires(*`order` and `s` consist of lowercase English letters.*);
	//@ requires(*All the characters in `order` are unique.*);
