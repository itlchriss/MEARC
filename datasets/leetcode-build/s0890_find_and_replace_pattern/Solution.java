...
public class Solution {
	...
	//@ requires(*The length of `pattern` is between 1 and 20 (inclusive).*);
	//@ requires(*The length of `words` is between 1 and 50 (inclusive).*);
	//@ requires(*Each string in `words` has the same length as `pattern`.*);
	//@ requires(*Each character in `pattern` and each string in `words` is a lowercase English letter.*);
