...
public class Solution {
	...
	//@ requires(*The length of `nums1` is at least 3.*);
	//@ requires(*The values in `nums1` and `nums2` are integers.*);
	//@ requires(*The values in `nums1` and `nums2` are between 0 and n-1, inclusive.*);
	//@ requires(*`nums1` and `nums2` are permutations of `[0, 1, ..., n - 1]`.*);
