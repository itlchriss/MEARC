...
public class Solution {
	...
	//@ requires(*The input `board` is a non-empty list of strings.*);
	//@ requires(*The length of each string in `board` is equal to the length of `board`.*);
	//@ requires(*The length of `board` is between 2 and 100 (inclusive).*);
	//@ requires(*Each string in `board` consists of characters 'E', 'X', 'S', or numeric characters '1' to '9'.*);
	//@ requires(*There is at least one 'E' character and one 'S' character in `board`.*);
	//@ requires(*There is no 'E' character or 'S' character in the same string as an 'X' character.*);
