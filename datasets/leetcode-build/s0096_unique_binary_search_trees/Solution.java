package g0001_0100.s0096_unique_binary_search_trees;

// #Medium #Top_100_Liked_Questions #Dynamic_Programming #Math #Tree #Binary_Tree
// #Binary_Search_Tree #Dynamic_Programming_I_Day_11 #Big_O_Time_O(n)_Space_O(1)
// #2022_06_21_Time_0_ms_(100.00%)_Space_40.4_MB_(72.43%)

public class Solution {
	//@ requires(*The input `n` is an integer.*);
	//@ requires(*`n` is greater than or equal to 1 and less than or equal to 19.*);
	//@ ensures(*The method returns an integer representing the number of structurally unique BSTs with `n` nodes.*);
	//@ ensures(*The returned value is greater than or equal to 1.*);
    public int numTrees(int n) {
        long result = 1;
        for (int i = 0; i < n; i++) {
            result *= 2L * n - i;
            result /= i + 1;
        }
        result /= n + 1;
        return (int) result;
    }
}