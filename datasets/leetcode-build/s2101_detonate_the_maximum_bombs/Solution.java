...
public class Solution {
	//@ requires(*The input `bombs` is a 2D integer array.*);
	//@ requires(*The length of `bombs` is between 1 and 100.*);
	//@ requires(*Each element in `bombs` is an array of length 3.*);
	//@ requires(*The values of `x_i`, `y_i`, and `r_i` in each element of `bombs` are between 1 and 10^5.*);
