...
public class Solution {
	...
	//@ requires(*The length of the input array `points` is greater than or equal to 1.*);
	//@ requires(*The coordinates `x` and `y` of each point in the input array `points` are within the range of -10^6 to 10^6.*);
	//@ requires(*All pairs of coordinates `(x, y)` in the input array `points` are distinct.*);
	//@ ensures(*The method returns an integer representing the minimum cost to make all points connected.*);
	//@ ensures(*All points in the input array `points` are connected.*);
	//@ ensures(*There is exactly one simple path between any two points in the input array `points`.*);
