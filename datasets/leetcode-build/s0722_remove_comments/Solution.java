...
public class Solution {
	//@ requires(*The input `source` is a non-null array of strings.*);
	//@ requires(*Each string in `source` represents a line of code in a C++ program.*);
	//@ requires(*The length of each string in `source` is between 0 and 80 characters.*);
	//@ requires(*Each character in `source` is a printable ASCII character.*);
	//@ requires(*There are no single-quote or double-quote characters in `source`.*);
	//@ requires(*Every open block comment in `source` is eventually closed.*);
