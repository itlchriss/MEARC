...
public class Solution {
    private static class RealNum {
        ...
        int orig;
        int real = 0;
	//@ requires(*The `mapping` array must have a length of 10.*);
	//@ requires(*Each element in the `mapping` array must be an integer between 0 and 9 (inclusive).*);
	//@ requires(*All elements in the `mapping` array must be unique.*);
	//@ requires(*The `nums` array must have a length greater than or equal to 1.*);
	//@ requires(*Each element in the `nums` array must be a non-negative integer less than 10^9.*);
