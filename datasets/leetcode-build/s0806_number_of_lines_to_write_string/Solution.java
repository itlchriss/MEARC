...
public class Solution {
	//@ requires(*The `widths` array must have a length of 26.*);
	//@ requires(*Each element in the `widths` array must be between 2 and 10 (inclusive).*);
	//@ requires(*The `s` string must have a length between 1 and 1000 (inclusive).*);
	//@ requires(*The `s` string must only contain lowercase English letters.*);
