...
public class Solution {
	//@ requires(*The input matrix `score` is not null.*);
	//@ requires(*The input matrix `score` has at least one row and one column.*);
	//@ requires(*The input matrix `score` contains distinct integers only.*);
	//@ requires(*The input integer `k` is a valid index within the range of the number of columns in `score`.*);
