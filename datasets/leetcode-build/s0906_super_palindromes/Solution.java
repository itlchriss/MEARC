...
public class Solution {
	//@ requires(*The input strings `left` and `right` must represent positive integers.*);
	//@ requires(*The length of `left` and `right` must be between 1 and 18 (inclusive).*);
	//@ requires(*`left` and `right` cannot have leading zeros.*);
