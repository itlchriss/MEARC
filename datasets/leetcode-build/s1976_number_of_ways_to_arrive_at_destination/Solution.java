...
public class Solution {
	...
	//@ requires(*The input `roads` is a 2D integer array.*);
	//@ requires(*The length of `roads` is less than or equal to `n * (n - 1) / 2`.*);
	//@ requires(*Each element `roads[i]` is an array of length 3.*);
	//@ requires(*The first element of `roads[i]`, `roads[i][0]`, is an integer between 0 and `n - 1`.*);
	//@ requires(*The second element of `roads[i]`, `roads[i][1]`, is an integer between 0 and `n - 1`.*);
	//@ requires(*The third element of `roads[i]`, `roads[i][2]`, is a positive integer between 1 and 10^9.*);
	//@ requires(*The first and second elements of `roads[i]` are not equal.*);
	//@ requires(*There is at most one road connecting any two intersections.*);
	//@ requires(*You can reach any intersection from any other intersection.*);
