package g0301_0400.s0386_lexicographical_numbers;

// #Medium #Depth_First_Search #Trie #2022_07_13_Time_4_ms_(93.65%)_Space_57.2_MB_(30.81%)

import java.util.ArrayList;
import java.util.List;

public class Solution {
	//@ requires(*The input `n` is a positive integer.*);
	//@ requires(*The input `n` is less than or equal to 50,000.*);
	//@ ensures(*The output is a list of integers.*);
	//@ ensures(*The output list contains all the numbers in the range `[1, n]` sorted in lexicographical order.*);
	//@ ensures(*The output list has a length of `n`.*);
	//@ ensures(*The output list is in ascending order.*);
    public List<Integer> lexicalOrder(int n) {
        List<Integer> lst = new ArrayList<>();
        dfs(lst, n, 0);
        return lst;
    }

    private void dfs(List<Integer> lst, int n, int num) {
        for (int i = 0; i <= 9; i++) {
            int cur = 10 * num + i;
            // get rid of 0
            if (cur == 0) {
                continue;
            }
            // when larger than n, return to the previous level
            if (cur > n) {
                return;
            }
            lst.add(cur);
            dfs(lst, n, cur);
        }
    }
}