...
public class Solution {
	//@ requires(*The input arrays `firstList` and `secondList` are not null.*);
	//@ requires(*The input arrays `firstList` and `secondList` are not empty.*);
	//@ requires(*The intervals in `firstList` and `secondList` are sorted in ascending order.*);
	//@ requires(*The intervals in `firstList` and `secondList` are pairwise disjoint.*);
