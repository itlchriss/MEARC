...
public class Solution {
	//@ requires(*1. The input `board` must be a non-null `m x n` matrix.*);
	//@ requires(*2. The input `word` must be a non-null string.*);
	//@ requires(*3. The length of `word` must be greater than 0.*);
	//@ requires(*4. The length of `word` must be less than or equal to the maximum of `m` and `n`.*);
	//@ requires(*5. Each element in `board` must be either `' '`, `'#'`, or a lowercase English letter.*);
