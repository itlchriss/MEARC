...
public class Solution {
	...
	//@ requires(*The length of the `words` array is equal to `n`.*);
	//@ requires(*The length of the `groups` array is equal to `n`.*);
	//@ requires(*Each string in the `words` array consists of lowercase English letters.*);
	//@ requires(*Each string in the `words` array has a length between 1 and 10 (inclusive).*);
	//@ requires(*Each element in the `groups` array is between 1 and `n` (inclusive).*);
	//@ requires(*The `words` array consists of distinct strings.*);
