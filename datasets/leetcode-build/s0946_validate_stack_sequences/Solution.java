...
public class Solution {
	//@ requires(*The `pushed` array and `popped` array must have the same length.*);
	//@ requires(*All elements in the `pushed` array must be unique.*);
	//@ requires(*The `pushed` array and `popped` array must have distinct values.*);
