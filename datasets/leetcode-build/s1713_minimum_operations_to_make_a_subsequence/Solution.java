...
public class Solution {
	//@ requires(*The input arrays `target` and `arr` are not null.*);
	//@ requires(*The length of `target` is greater than or equal to 1.*);
	//@ requires(*The length of `arr` is greater than or equal to 1.*);
	//@ requires(*The elements in `target` are distinct.*);
	//@ requires(*The elements in `target` and `arr` are integers.*);
	//@ requires(*The elements in `target` and `arr` are within the range of 1 to 10^9.*);
	//@ requires(*`target` does not contain any duplicates.*);
