...
public class Solution {
	//@ requires(*The `products` array is not null.*);
	//@ requires(*The `searchWord` string is not null.*);
	//@ requires(*The `products` array has at least one element.*);
	//@ requires(*Each element in the `products` array is a non-empty string consisting of lowercase English letters.*);
	//@ requires(*The length of each element in the `products` array is between 1 and 3000.*);
	//@ requires(*The sum of the lengths of all elements in the `products` array is between 1 and 20000.*);
	//@ requires(*The `searchWord` string consists of lowercase English letters.*);
	//@ requires(*The length of the `searchWord` string is between 1 and 1000.*);
