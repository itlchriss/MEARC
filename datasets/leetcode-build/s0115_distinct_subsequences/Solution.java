...
public class Solution {
	...
	//@ requires(*The lengths of `s` and `t` are between 1 and 1000, inclusive.*);
	//@ requires(*`s` and `t` consist of English letters.*);
	//@ ensures(*The method returns an integer representing the number of distinct subsequences of `s` that equal `t`.*);
