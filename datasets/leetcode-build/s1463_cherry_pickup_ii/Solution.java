...
public class Solution {
	...
	//@ requires(*The number of rows and columns in the grid must be between 2 and 70 (inclusive).*);
	//@ requires(*The values in the grid must be between 0 and 100 (inclusive).*);
	//@ ensures(*The method must return an integer representing the maximum number of cherries collected by both robots.*);
	//@ ensures(*The grid must be modified such that all cells passed by the robots become empty cells.*);
	//@ ensures(*The robots must reach the bottom row of the grid.*);
