...
public class Solution {
	//@ requires(*1. The input values `hour` and `minutes` must be integers.*);
	//@ requires(*2. The value of `hour` must be between 1 and 12 (inclusive).*);
	//@ requires(*3. The value of `minutes` must be between 0 and 59 (inclusive).*);
