...
public class Solution {
	...
	//@ requires(*The length of the input array `height` is greater than or equal to 1.*);
	//@ requires(*The elements in the input array `height` are non-negative integers.*);
	//@ requires(*The width of each bar in the elevation map is 1.*);
	//@ ensures(*The method returns an integer representing the amount of water that can be trapped.*);
	//@ ensures(*The returned value is greater than or equal to 0.*);
	//@ ensures(*The returned value is the sum of the units of rain water trapped in the elevation map.*);
