...
public class Solution {
	...
	//@ requires(*The input array `w` is not null.*);
	//@ requires(*The length of the input array `w` is greater than or equal to - Each element in the input array `w` is a positive integer.*);
	//@ requires(*The sum of all elements in the input array `w` is not zero.*);
	//@ ensures(*The method returns an integer index in the range `[0, w.length - 1]`.*);
	//@ ensures(*The probability of picking the returned index is `w[index] / sum(w)`.*);
