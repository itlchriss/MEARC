...
public class Solution {
	...
	//@ requires(*The input matrix `mat` has at least one row and one column.*);
	//@ requires(*The input matrix `mat` contains only 0s and 1s.*);
	//@ ensures(*The output is an integer representing the minimum number of steps required to convert `mat` to a zero matrix.*);
	//@ ensures(*If it is not possible to convert `mat` to a zero matrix, the output is -1.*);
