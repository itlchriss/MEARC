...
//@ ensures(*1. The input array `points` must have a length of 3.*);
//@ ensures(*2. Each element in the `points` array must have a length of 2.*);
//@ ensures(*3. The values of `x` and `y` in each point must be between 0 and 100 (inclusive).*);
//@ ensures(**);
