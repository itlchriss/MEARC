...
//@ ensures(*The length of the input array `points` is at least - The elements in the input array `points` are not null.*);
//@ ensures(*The length of each element in the input array `points` is - The value of `k` is non-negative.*);
//@ ensures(*The elements in the input array `points` are sorted in ascending order based on the x-values.*);
//@ ensures(*The x-values in the input array `points` form a strictly increasing sequence.*);
//@ ensures(**);
