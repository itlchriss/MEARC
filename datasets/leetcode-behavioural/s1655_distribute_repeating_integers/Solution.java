...
//@ ensures(*The input arrays `nums` and `quantity` are not null.*);
//@ ensures(*The length of `nums` is equal to `n`.*);
//@ ensures(*The length of `quantity` is equal to `m`.*);
//@ ensures(*The values in `nums` are integers between 1 and 1000.*);
//@ ensures(*The values in `quantity` are integers between 1 and 100000.*);
//@ ensures(*The number of unique values in `nums` is at most 50.*);
//@ ensures(**);
