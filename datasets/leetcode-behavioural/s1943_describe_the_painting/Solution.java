...
//@ ensures(*The input `segments` is a 2D integer array.*);
//@ ensures(*Each element `segments[i]` is an array of length 3.*);
//@ ensures(*The start value `start_i` of each segment is less than the end value `end_i`.*);
//@ ensures(*The color value `color_i` of each segment is distinct.*);
//@ ensures(**);
