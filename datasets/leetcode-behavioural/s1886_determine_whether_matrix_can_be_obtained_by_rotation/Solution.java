...
//@ ensures(*1. The input matrices `mat` and `target` must have the same dimensions.*);
//@ ensures(*2. The dimensions of `mat` and `target` must be square matrices (n x n).*);
//@ ensures(*3. The values in `mat` and `target` must be either 0 or 1.*);
//@ ensures(**);
