...
//@ ensures(*The input arrays `watchedVideos` and `friends` must have the same length `n`.*);
//@ ensures(*The length of each element in `watchedVideos` must be greater than or equal to 1 and less than or equal to 100.*);
//@ ensures(*The length of each element in `watchedVideos` must be greater than or equal to 1 and less than or equal to 8.*);
//@ ensures(*The length of each element in `friends` must be less than `n`.*);
//@ ensures(*The value of `id` must be between 0 and `n-1`.*);
//@ ensures(*The value of `level` must be between 1 and `n-1`.*);
//@ ensures(*If `friends[i]` contains `j`, then `friends[j]` must contain `i`.*);
//@ ensures(**);
