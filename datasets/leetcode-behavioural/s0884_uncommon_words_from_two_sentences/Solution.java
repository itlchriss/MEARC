...
//@ ensures(*2. The lengths of `s1` and `s2` are between 1 and 200 (inclusive).*);
//@ ensures(*3. `s1` and `s2` consist only of lowercase English letters and spaces.*);
//@ ensures(*4. `s1` and `s2` do not have leading or trailing spaces.*);
//@ ensures(*5. All the words in `s1` and `s2` are separated by a single space.*);
//@ ensures(**);
