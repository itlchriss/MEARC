...
//@ ensures(*The input `points` is a 2D array of integers.*);
//@ ensures(*The length of `points` is greater than or equal to 2.*);
//@ ensures(*Each element in `points` is an array of length 2.*);
//@ ensures(*The values of `x` and `y` in each element of `points` are non-negative integers.*);
//@ ensures(**);
