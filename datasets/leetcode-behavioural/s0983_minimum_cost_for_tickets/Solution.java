...
//@ ensures(*The input arrays `days` and `costs` are not null.*);
//@ ensures(*The length of `days` is greater than or equal to 1 and less than or equal to 365.*);
//@ ensures(*Each element in `days` is an integer between 1 and 365.*);
//@ ensures(*The elements in `days` are in strictly increasing order.*);
//@ ensures(*The length of `costs` is 3.*);
//@ ensures(*Each element in `costs` is an integer between 1 and 1000.*);
//@ ensures(**);
