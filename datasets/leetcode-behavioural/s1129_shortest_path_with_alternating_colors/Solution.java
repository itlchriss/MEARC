...
//@ ensures(*The arrays `redEdges` and `blueEdges` are not null.*);
//@ ensures(*The length of `redEdges` is not greater than 400.*);
//@ ensures(*The length of `blueEdges` is not greater than 400.*);
//@ ensures(*Each element in `redEdges` is an array of length 2.*);
//@ ensures(*Each element in `blueEdges` is an array of length 2.*);
//@ ensures(*The values in `redEdges` and `blueEdges` are within the range of 0 to `n-1`.*);
//@ ensures(**);
