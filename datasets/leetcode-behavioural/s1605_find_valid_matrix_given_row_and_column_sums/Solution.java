...
//@ ensures(*The lengths of `rowSum` and `colSum` arrays are equal.*);
//@ ensures(*The lengths of `rowSum` and `colSum` arrays are not greater than 500.*);
//@ ensures(*The elements of `rowSum` and `colSum` arrays are non-negative integers.*);
//@ ensures(*The sum of all elements in `rowSum` array is equal to the sum of all elements in `colSum` array.*);
//@ ensures(**);
