...
//@ ensures(*The input arrays `startWords` and `targetWords` are not null.*);
//@ ensures(*The length of `startWords` is greater than or equal to - The length of `targetWords` is greater than or equal to - Each string in `startWords` and `targetWords` consists of lowercase English letters only.*);
//@ ensures(*No letter occurs more than once in any string of `startWords` or `targetWords`.*);
//@ ensures(**);
