...
//@ ensures(*The input arrays `rooms` and `queries` are not null.*);
//@ ensures(*The length of `rooms` is equal to `n`.*);
//@ ensures(*The length of `queries` is equal to `k`.*);
//@ ensures(*Each element in `rooms` is an array of length 2.*);
//@ ensures(*Each element in `queries` is an array of length 2.*);
//@ ensures(*The values of `roomId_i` in `rooms` are unique.*);
//@ ensures(*The values of `roomId_j` in `queries` are within the range of 1 to 10^7.*);
//@ ensures(*The values of `size_i` in `rooms` and `minSize_j` in `queries` are within the range of 1 to 10^7.*);
//@ ensures(**);
