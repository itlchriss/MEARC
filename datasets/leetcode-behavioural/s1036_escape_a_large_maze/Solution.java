...
//@ ensures(*1. The `blocked` array is not null.*);
//@ ensures(*2. The `source` array is not null.*);
//@ ensures(*3. The `target` array is not null.*);
//@ ensures(*4. The `blocked` array does not contain any null elements.*);
//@ ensures(*5. The `source` array contains exactly two elements.*);
//@ ensures(*6. The `target` array contains exactly two elements.*);
//@ ensures(*7. The `blocked` array does not contain any duplicate elements.*);
//@ ensures(*8. The `source` and `target` arrays do not contain any duplicate elements.*);
//@ ensures(*9. The `blocked` array does not contain the `source` or `target` elements.*);
//@ ensures(*10. The `blocked` array contains at most 200 elements.*);
//@ ensures(*11. The coordinates in the `blocked` array are within the range of 0 to 1 million.*);
//@ ensures(*12. The coordinates in the `source` and `target` arrays are within the range of 0 to 1 million.*);
//@ ensures(*13. The `source` and `target` arrays are not equal.*);
//@ ensures(**);
