...
//@ ensures(*1. The lengths of `s1` and `s2` must be between 1 and 40 (inclusive).*);
//@ ensures(*2. `s1` and `s2` must consist of lowercase English letters and digits 1-9 (inclusive).*);
//@ ensures(*3. The number of consecutive digits in `s1` and `s2` must not exceed 3.*);
//@ ensures(**);
