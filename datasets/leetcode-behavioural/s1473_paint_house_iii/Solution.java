...
//@ ensures(*1. The length of the `houses` array must be equal to the length of the `cost` array.*);
//@ ensures(*2. The length of each row in the `cost` matrix must be equal to `n`.*);
//@ ensures(*3. The values in the `houses` array must be between 0 and `n`, inclusive.*);
//@ ensures(*4. The value of `m` must be between 1 and 100, inclusive.*);
//@ ensures(*5. The value of `n` must be between 1 and 20, inclusive.*);
//@ ensures(*6. The value of `target` must be between 1 and `m`, inclusive.*);
//@ ensures(*7. The values in the `cost` matrix must be between 1 and 10^4, inclusive.*);
//@ ensures(**);
