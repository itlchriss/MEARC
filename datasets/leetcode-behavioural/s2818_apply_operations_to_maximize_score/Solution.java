...
//@ ensures(*The input list `nums` must contain at least one element.*);
//@ ensures(*The input integer `k` must be greater than or equal to - The input integer `k` must be less than or equal to the minimum of `n * (n + 1) / 2` and `10^9`, where `n` is the length of `nums`.*);
//@ ensures(**);
