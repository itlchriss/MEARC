...
//@ ensures(*Each element in the input array `stones` is not null.*);
//@ ensures(*Each element in the input array `stones` is an array of length 2.*);
//@ ensures(*The coordinates `x` and `y` of each stone in the input array `stones` are non-negative integers.*);
//@ ensures(*No two stones in the input array `stones` have the same coordinate point.*);
//@ ensures(**);
