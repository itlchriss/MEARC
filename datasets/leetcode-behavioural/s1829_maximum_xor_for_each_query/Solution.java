...
//@ ensures(*The input array `nums` is sorted in ascending order.*);
//@ ensures(*The length of the input array `nums` is `n`.*);
//@ ensures(*The value of `n` is between 1 and 10^5 (inclusive).*);
//@ ensures(*The value of `maximumBit` is between 1 and 20 (inclusive).*);
//@ ensures(*Each element in the input array `nums` is a non-negative integer between 0 and 2^maximumBit - 1 (inclusive).*);
//@ ensures(**);
