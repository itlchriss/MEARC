...
//@ ensures(*The input array `richer` is not null.*);
//@ ensures(*The input array `quiet` is not null.*);
//@ ensures(*The length of `quiet` is equal to the number of people `n`.*);
//@ ensures(*The values in `quiet` are unique.*);
//@ ensures(*The values in `quiet` are integers between 0 and `n-1`.*);
//@ ensures(*The length of `richer` is less than or equal to `n * (n - 1) / 2`.*);
//@ ensures(*The pairs in `richer` are unique.*);
//@ ensures(*The values in `richer` are integers between 0 and `n-1`.*);
//@ ensures(*The values in `richer` do not contain any pairs where `a_i` is equal to `b_i`.*);
//@ ensures(*The observations in `richer` are logically consistent.*);
//@ ensures(**);
