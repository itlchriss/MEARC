...
//@ ensures(*2. The input matrix `mat` has at least one row.*);
//@ ensures(*3. The input matrix `mat` has at least one column.*);
//@ ensures(*4. The input matrix `mat` has the same number of columns for each row.*);
//@ ensures(*5. The target value `target` is a positive integer.*);
//@ ensures(*6. The target value `target` is less than or equal to 800.*);
//@ ensures(**);
