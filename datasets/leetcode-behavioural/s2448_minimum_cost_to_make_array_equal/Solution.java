...
//@ ensures(*The length of `nums` and `cost` arrays must be the same.*);
//@ ensures(*The length of `nums` and `cost` arrays must be greater than or equal to 1.*);
//@ ensures(*All elements in `nums` and `cost` arrays must be positive integers.*);
//@ ensures(*The maximum value of `n` is 10^5.*);
//@ ensures(*The maximum value of each element in `nums` and `cost` arrays is 10^6.*);
//@ ensures(**);
