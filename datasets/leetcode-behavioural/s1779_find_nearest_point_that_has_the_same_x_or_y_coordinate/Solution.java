...
//@ ensures(*The input integers `x` and `y` must be valid Cartesian coordinates.*);
//@ ensures(*The input array `points` must not be null.*);
//@ ensures(*The length of the input array `points` must be greater than or equal to 1.*);
//@ ensures(*Each element in the input array `points` must be an array of length 2.*);
//@ ensures(**);
