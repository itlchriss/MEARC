...
//@ ensures(*The input arrays `startPos`, `homePos`, `rowCosts`, and `colCosts` are not null.*);
//@ ensures(*The length of `rowCosts` is equal to `m`.*);
//@ ensures(*The length of `colCosts` is equal to `n`.*);
//@ ensures(*The values in `rowCosts` and `colCosts` are non-negative integers.*);
//@ ensures(*The values in `startPos` and `homePos` are valid cell coordinates within the grid.*);
//@ ensures(**);
