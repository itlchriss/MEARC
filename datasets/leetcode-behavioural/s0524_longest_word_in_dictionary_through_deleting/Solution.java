...
public class Solution {
    private static class Pair {
        private String word;
        private int index;
//@ ensures(*If there is a string `s` and a string array `dictionary`, the result is the longest string in the dictionary that can be formed by deleting some characters from the string `s`.*);
//@ ensures(*If there are multiple possible results, the result is the longest word with the smallest lexicographical order.*);
//@ ensures(*If there is no possible result, the result is an empty string.*);
