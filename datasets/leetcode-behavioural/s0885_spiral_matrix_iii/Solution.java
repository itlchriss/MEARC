...
//@ ensures(*The input values `rows` and `cols` must be integers greater than or equal to 1.*);
//@ ensures(*The input values `rStart` and `cStart` must be integers greater than or equal to 0 and less than `rows` and `cols` respectively.*);
//@ ensures(**);
